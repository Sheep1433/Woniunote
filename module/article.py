...
from common.demo import create_es_data
...
class Article(DBase):
    ...
    __searchable__ = [('headline', True), ('content', True)]
    ...
    def find_by_userid(self):
        result = dbsession.query(Article).filter(Article.userid == session.get('userid'),
                                                 Article.hidden == 0, Article.drafted == 0,
                                                 Article.checked == 1).order_by(Article.updatetime.desc()
                                                                                ).all()
        return result
    ...
    # 根据文章标题和内容进行模糊搜索
    # 调用esfinal中的query方法从ES中获取搜索到的文章id结果集
    # 根据文章articleid返回result
    def find_by_head_content(self, index, query, page, per_page):
        from common.elasticsearchfinal import query_index
        total, results, highlights = query_index(index, query, page, per_page)
        result = []
        for i in results:
            articleid = i[0]
            res = dbsession.query(Article, Users.nickname).join(Users, Users.userid == Article.userid) \
                .filter(Article.hidden == 0, Article.drafted == 0, Article.checked == 1,
                       Article.articleid == articleid).first()
            if res:
                result.append(res)
        return total, result, highlights
    ...
    def find_last_9(self):
        result = dbsession.query(Article.articleid, Article.headline).filter(Article.hidden == 0,
                                                                             Article.drafted == 0,
                                                                             Article.checked == 1).order_by(
            Article.articleid.desc()).limit(9).all()
        return result
    ...
    def find_most_9(self):
        result = dbsession.query(Article.articleid, Article.headline).filter(Article.hidden == 0,
                                                                             Article.drafted == 0,
                                                                             Article.checked == 1).order_by(
            Article.readcount.desc()).limit(9).all()
        return result
    ...
    def find_recommended_9(self):
        result = dbsession.query(Article.articleid, Article.headline). \
            filter(Article.hidden == 0,
                   Article.drafted == 0, Article.checked == 1, Article.recommended == 1). \
...
# article = Article()
# article.find_last_most_recommended()
